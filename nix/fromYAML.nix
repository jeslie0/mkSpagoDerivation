# Graciously provided by github.com/thomashoneyman
  ...
  fromYAML =
    ...
    let
      ...
      filtered =
        ...
        in
        if noEmpties == [ ] then throw "Empty YAML file" else noEmpties;
      ...
      parsePrimitiveElem =
        ...
        if elem == "[]" then
          ...
          { }
        else if elem == "true" then
          true
        else if elem == "false" then
          false
        else
          elem;
      ...
      matchLine =
        ...
        in
        if listEntry != null then
          {
            isListEntry = true;
            indent = (builtins.stringLength (builtins.elemAt listEntry 0)) / 2;
            key = if singleLine != null then builtins.elemAt singleLine 1 else null;
            value = if singleLine != null then builtins.elemAt singleLine 2 else builtins.elemAt listEntry 1;
          }
        # Handle single line key -> val assignments
        else if singleLine != null then
          {
            isListEntry = false;
      ...
      processLines =
        ...
        let
          ...
          indent = if index == -1 then -1 else match.indent;
          ...
          findChildIndices =
            ...
            in
            if searchIndex >= numLines then
              [ ]
            else if matchSearch.indent > childIndent then
              findChildIndices (searchIndex + 1)
            else if matchSearch.indent == childIndent then
              if matchSearch.isListEntry == childrenMustBeList then
                [ searchIndex ] ++ findChildIndices (searchIndex + 1)
              else
                findChildIndices (searchIndex + 1)
            else
              [ ];

          childIndent = if nextMatch.indent > indent then nextMatch.indent else null;
          ...
          childrenMerged = lib.foldl (
            all: currentObject:
            (
              if builtins.isAttrs currentObject then
                (if all == null then { } else all) // currentObject
              else
                (if all == null then [ ] else all) ++ currentObject
            )
          ) null childObjects;
          ...
          result =
            if index == (-1) then
              childrenMerged
            else if match.isListEntry then
              # Has key and value -> check if attr continue
              if match.key != null && match.value != null then
                # Attrs element follows
                if match.indent == nextMatch.indent && !nextMatch.isListEntry then
                  [
